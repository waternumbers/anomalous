...
The purpose of this vignette is to present the CROPS adopted for anomalies.

In the follow variables identified by Greek letters are considered known *a priori*.

Suppose there are ordered data $y_{t \in T} = y_{1},\ldots,\tau$ which have some background statistical model parameterisated by $\Theta$.
The cost associates with representing the data by the model is written
\[
C\left(
...
## Point anomaly - Not happy with this
...
To acheive this the modified cost of a point anomaly includes a positive constant $\gamma$ and is expressed as
\[
C_{P}\left(y_{t} \left| \lambda_t,r_k\right.\right) = 
\gamma + 2 y_{t}
- 2 y_{t} \log\left(y_{k}\right)
