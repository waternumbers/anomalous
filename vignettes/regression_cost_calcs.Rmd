...
The likelihood of of the $n_{t}$ observations in $\mathbf{y}_{t}$ being
\[
L\left(\mathbf{y}_{t} \left| \theta_k,\sigma_k\right.\right) = 
\left(2\pi \sigma_{k}\right)^{-n_{t}/2} 
\det\left(\mathbf{S}_{t}\right)^{-1/2}
\exp\left(-\frac{1}{2\sigma_{k}}\left( \mathbf{y}_{t} - \mathbf{X}_{t} \mathbf{m}_{t} - \mathbf{X}_{t} \theta_{k}\right)^{\prime} \mathbf{S}_{t}^{-1} \left( \mathbf{y}_{t}  - \mathbf{X}_{t} \mathbf{m}_{t} - \mathbf{X}_{t} \theta_{k}\right) \right)
\]
With $\hat{\mathbf{y}}_{t} = \mathbf{y}_{t} - \mathbf{X}_{t} \mathbf{m}_{t}$ the log likelihood is given by
\[
l\left(\mathbf{y}_{t} \left| \theta_k,\sigma_k \right.\right) = 
-\frac{n_{t}}{2} \log\left(2\pi \sigma_{k}\right)
-\frac{1}{2} \log\left( \det\left(\mathbf{S}_{t}\right)\right)
-\frac{1}{2\sigma_{k}}\left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)^{\prime} \mathbf{S}_{t}^{-1} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)
\]
or more simply
\[
l\left(\mathbf{y}_{t} \left| \theta_k,\sigma_k \right.\right) = 
-\frac{1}{2}K_{t} 
-\frac{n_{t}}{2} \log\left(\sigma_{k}\right)
-\frac{1}{2\sigma_{k}}\left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)^{\prime} \mathbf{S}_{t}^{-1} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)
\]
...
-\frac{n_{k}}{2} \log\left(\sigma_{k}\right)
-\frac{1}{2\sigma_{k}}\sum_{t \in T_{k}} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)^{\prime} \mathbf{S}_{t}^{-1} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)
\]
...
\sum_{t \in T_{k}} K_{t} 
+n_{k} \log\left(\sigma_{k}\right)
+\frac{1}{\sigma_{k}}\sum_{t \in T_{k}} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)^{\prime} \mathbf{S}_{t}^{-1} \left( \hat{\mathbf{y}}_{t} - \mathbf{X}_{t} \theta_{k}\right)
+\beta
\]
...
Here $\theta_{k}=0$ and $\sigma_{k}=1$ and there is no penalty so $\beta = 0$. This gives

\[
C_{B}\left(\mathbf{y}_{t \in T_{k}} \left| \mu_t,m_k,\sigma_k,s_k\right.\right) = 
\sum_{t \in T_{k}} K_{t} 
+ \sum_{t \in T_{k}} \hat{\mathbf{y}}_{t}^{\prime} \mathbf{S}_{t}^{-1} \hat{\mathbf{y}}_{t}
\]
