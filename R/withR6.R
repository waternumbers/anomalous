...
system.time({ tmp <- rep(NA,length(x)); for(ii in 1:length(x)){tmp[ii] <- fCost$collective(ii,length(x))}})
system.time({ tmp2 <- rep(NA,length(x)); for(ii in 1:length(x)){tmp[ii] <- fCost$collective2(ii,length(x))}})
...
addCollective <- function(p,x,s,e){
    ...
    ##    tmp <- data.frame(start = s,end = e,cost = cst)
    ##    p$ca <- rbind(p$ca, tmp)
    p$cost <- p$cost + cst
    p$last_time <- e
    return(p)
...
addPoint <- function(p,x,s){
    ...
    ##tmp <- data.frame(location=s, cost = cst)
    ##p$pa <- rbind(p$pa,tmp)
    p$cost <- p$cost + cst
    p$last_time <- s
    return(p)
